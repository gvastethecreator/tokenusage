...
A skill whose client policy allows the agent to select it from discovery metadata. The human can still type its name, so implicit invocation adds agent reach without removing explicit reach. It spends discovery **context load** in exchange for autonomous routing. In Codex, omit `policy.allow_implicit_invocation` or set it to `true` in `agents/openai.yaml`. Pick it only when the agent or another workflow must reach the skill without the human selecting `$skill` first.

_Avoid_: ability, tool, capability

### Explicit-Only

A skill the client will not select from an ordinary user prompt; the human reaches it by typing `$skill`. It reduces accidental activation and may remove its discovery metadata from the model's default context, depending on the client. In Codex, keep the required `description` and set `policy.allow_implicit_invocation: false` in `agents/openai.yaml`.

_Avoid_: procedure, workflow, command

### Description

The required machine-readable summary of what a skill does and when it applies. For an implicitly invoked skill it is the top-level **context pointer** used during routing. Its presence does not choose invocation policy; client configuration does. In Codex, that policy lives in `agents/openai.yaml`.
...
The tokens and attention spent advertising an implicitly invoked skill's name, **description**, path, or other discovery metadata. Clients may truncate or omit entries as the catalog grows, so every new implicit skill competes for routing attention. Explicit-only skills can avoid some default discovery cost when the client honors that policy.

_Avoid_: token cost, context bloat

### Cognitive Load

The cost an explicit-only skill imposes on the human — what they must hold in their head: which skills exist and when to select each. Implicit invocation reduces that burden through agent discovery. Not a cost to minimise blindly: it is the price of deliberate human routing where accidental activation would be worse.

_Avoid_: human index, burden, overhead

### Router Skill

A skill whose job is to name related explicit-only skills and tell the human when to select each, so the human remembers one entrypoint instead of many. Whether the router itself is implicit or explicit is a separate policy choice. It cures **cognitive load** without making every specialist implicitly invocable.

_Avoid_: dispatcher, menu, registry, index, router procedure

### Granularity

How finely you divide skills. Finer division spends one of the two loads: more implicitly invoked skills spend **context load** and compete for routing attention; more explicit-only skills spend **cognitive load** because the human must remember them. Two cuts guide the division. By **invocation**, split off an implicit skill where a distinct **leading word** should trigger it. By **sequence**, split a run of **steps** where a step's **post-completion steps** need hiding, since isolating it in a fresh context clears what follows. Beware the reverse: merging sequences exposes later steps and can invite premature completion.
...
A skill with no **steps** uses just the bottom two rungs — often a legitimately flat peer-set (e.g. every rule of a review on one rung), which is a fine arrangement, not a smell. The hierarchy is independent of invocation: a skill can be implicit or explicit-only whether it is all steps, all reference, or both. When a skill has steps, in-file reference that should be disclosed buries them and turns attending to them into a coin-flip — a variance lever, not just a legibility one. Keep the top of the ladder legible; push down it whatever you can.
...
**Reference** that lives outside the skill system — a plain file, no **description**, no **steps**, not invocable — that any skill can point at. It is the home for shared material that need not trigger independently and avoids paying discovery cost for another skill.
...
_Failure mode._ Ending the current step before it is genuinely done, because the agent's attention slips to being done rather than to the work. A between-steps failure: it needs **steps** to occur — a skill with no steps that quits early isn't premature completion but thin **legwork** under an unmet demand. A tug-of-war between two forces: visible **post-completion steps** (the pull forward) and the **completion criterion**'s clarity (the resistance — a sharp, checkable bar holds; a vague one gives way). Fuzziness is the necessary condition: a sharp bound resists the pull no matter how many later steps are visible, so a step that never rushes needs no defending. Two levers hold a step that does, but reach for them in order: **sharpen the bound first** — it is local and cheap. Only when the criterion is irreducibly fuzzy _and_ you actually observe the rush do you **hide the later steps** — and hiding only works across a real context boundary such as an explicit hand-off or fresh subagent; an inline call leaves later steps in context. One cause of thin legwork, but distinct from it: legwork can be thin even when a step runs to full completion.

_Avoid_: premature closure, the rush, rushing, shortcutting

### Negation

_Failure mode._ Steering by prohibition: telling the agent what _not_ to do, which names the forbidden behavior into context and can make it more available. The cure is the positive target: describe the behavior to produce. A prohibition earns its place only as a hard guardrail; pair it with the target so attention lands on what to do.

_Avoid_: ironic rebound, don't-prompting, pink-elephant steering

### Negative Space

_Failure mode._ Leaving a decision unstated and assuming the silence is neutral. The model fills the gap from its priors, so an omitted branch can become accidental policy. Read a skill for its silences: fill the gap with the intended behavior, or mark it as an explicit open branch with an owner and stopping condition.

_Avoid_: silent delegation, unspecified default, accidental branch
