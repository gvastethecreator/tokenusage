...
Which local sources can WOpenUsage use for Codex, Grok Build, and OpenCode, and why do they fail to reach the spend donut today?
...
- [Codex issue 23340](https://github.com/openai/codex/issues/23340) documents inflated `threads.tokens_used` values when nested agent spans share a process. WOpenUsage does not use that column.
...
- The [OpenAI model comparison](https://developers.openai.com/api/docs/models/compare) lists the per-million-token prices for GPT-5.6 Sol, Terra, and Luna. The [GPT-5.4 mini model page](https://developers.openai.com/api/docs/models/gpt-5.4-mini) provides its input, cached-input, and output rates. WOpenUsage labels values derived from these prices as estimates because plan charges can differ.
...
Use the OpenUsage source shape where it matches the installed Windows data: Grok reads `unified.jsonl` first and OpenCode queries SQLite by date. Codex keeps `account/usage/read` as the total because the local corpus is large, child rollouts can replay counters, and ephemeral threads may lack files; bounded session tails only provide model mix. Persist normalized rollups in WOpenUsage SQLite and publish that cache before starting a fresh scan. Register all three as local usage sources. Remove Vercel AI Gateway from active composition and visible options while retaining its code for later work.
