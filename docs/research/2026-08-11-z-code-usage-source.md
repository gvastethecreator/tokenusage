...
| Per-call input, output, cached, and total tokens | Yes | The public Z.ai Chat Completion response returns documented JSON counters. | Stable for the individual API request made by that client. It is not ZCode history. | Not suitable for the ZCode provider. A separate Z.ai integration could use a user-supplied key. |
